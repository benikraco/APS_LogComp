...
reserved_words = ["imprima", "se", "fim", "senao", "enquanto", "leia", "Int", "String", "retorna", "funcao"]
...
class Parser:
    ...
    def ParseFactor(tokens):
        ...
        elif tokens.next.type == "RESERVED" and tokens.next.value == "leia":
            tokens.selectNext()
            if tokens.next.type == "PAR_OPEN":
                tokens.selectNext()
                if tokens.next.type == "PAR_CLOSE":
    ...
    def ParseStatement(tokens):
        ...
        # verify print
        elif tokens.next.type == "RESERVED" and tokens.next.value == "imprima":
            tokens.selectNext()
        ...
        # verify while
        elif tokens.next.type == "RESERVED" and tokens.next.value == "enquanto":
            tokens.selectNext()
            ...
            if tokens.next.type == "NEWLINE":
                ...
                # verify end
                while tokens.next.value != "fim":
                    blo1.children.append(Parser.ParseStatement(tokens))
                if tokens.next.type == "RESERVED" and tokens.next.value == "fim":
                    tokens.selectNext()
                    return While("ENQUANTO", [var, blo1])
                
        elif tokens.next.type == "RESERVED" and tokens.next.value == "se":
            tokens.selectNext()
            var = Parser.ParseRelExpression(tokens)
            if tokens.next.type == "NEWLINE":
                tokens.selectNext()
                ...
                while control:
                    if tokens.next.type == "NEWLINE":
                        new = True
                        tokens.selectNext()
                    if new and (tokens.next.value == "senao" or tokens.next.value == "fim"):
                        control = False
                    if control:
                        blo1.children.append(Parser.ParseStatement(tokens))

                if tokens.next.type == "RESERVED" and tokens.next.value == "senao":
                    tokens.selectNext()
                    if tokens.next.type == "NEWLINE":
                        tokens.selectNext()
                        ...
                        if tokens.next.type == "RESERVED" and tokens.next.value == "fim":
                            tokens.selectNext()
                            return If("SE", [var, blo1, else_blo])
                        else:
                            sys.stderr.write("[ERROR - ParseStatement] - Missing end")
                            sys.exit()

                elif tokens.next.type == "RESERVED" and tokens.next.value == "fim":
                    tokens.selectNext()
                    return If("SE", [var, blo1])
                else:
                    sys.stderr.write("[ERROR - ParseStatement] - Missing end")
                    sys.exit()
        ...
        # verify function
        elif tokens.next.type == "RESERVED" and tokens.next.value == "funcao":
            tokens.selectNext()
        ...
        # verify return
        elif tokens.next.type == "RESERVED" and tokens.next.value == "retorna":
            tokens.selectNext()
            
            node = Return("RETORNA", [Parser.ParseRelExpression(tokens)])
